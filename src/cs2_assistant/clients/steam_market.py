...
def _hmac_sha1_base64(secret_b64: str, message: str) -> str:
...
class SteamMarketClient:
    def __init__(
        ...
        *,
        ...
        if not steam_id64:
            raise SteamMarketError("missing Steam ID64")
    ...
    def list_active_listings(self, *, start: int = 0, count: int = 100) -> list[SteamListing]:
        ...
        listings: dict[str, Any] = payload.get("listings") or {}
        ...
        for listing_id, listing in listings.items():
            ...
            if isinstance(listing, dict):
                ...
                asset_id = str(asset_info.get("id") or "") or None
                mhn = str(listing.get("asset", {}).get("market_hash_name") or "") or None
    ...
    def fetch_confirmations(self) -> list[dict[str, Any]]:
        ...
        now = int(time.time())
        key = _hmac_sha1_base64(self.identity_secret, f"{now}conf")
        params = {
            "p": self.device_id,
            "a": self.steam_id64,
            "k": key,
    ...
    def confirm_all(self) -> int:
        ...
        now = int(time.time())
        key = _hmac_sha1_base64(self.identity_secret, f"{now}allow")
        params = {
            "p": self.device_id,
            "a": self.steam_id64,
            "k": key,
        ...
        for idx, conf in enumerate(confirmations):
            cid = str(conf.get("id"))
            ckey = str(conf.get("nonce"))
            data[f"cid[{idx}]"] = cid
            data[f"ck[{idx}]"] = ckey
        response = self._request("POST", "/mobileconf/multiajaxop", params=params, data=data)
        try:
            payload = response.json()
        except ValueError as exc:
