export type ProfitTradeLongBuyStrategyConfig = {
  enabled: boolean;
...
export type ProfitTradeLongBuyConfigKey =
  | "enabled"
  | "allowRealExecution"
  | "longBuyEnabled"
  | "longBuyAllowRealExecution";

export function requiresLongBuyConfigConfirmation(key: ProfitTradeLongBuyConfigKey): boolean {
  return key === "longBuyAllowRealExecution";
}

/** The total switch owns both the persistent config and the runtime schedule. */
export function usesProfitTradeRuntimeToggle(key: ProfitTradeLongBuyConfigKey): boolean {
  return key === "enabled";
}

export type ProfitTradeLongBuyStrategyMode = "disabled" | "observe" | "live";

export type ProfitTradeLongBuyStrategyState = {
  mode: ProfitTradeLongBuyStrategyMode;
  canObserve: boolean;
  canReconcileExistingOrders: boolean;
  canWriteSteam: boolean;
  canExecuteC5Followup: boolean;
  label: string;
  detail: string;
};
 ...
 */
export function resolveProfitTradeLongBuyStrategyState(
