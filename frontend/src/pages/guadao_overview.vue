  ...
  <main class="page overview-page">
    ...
    <section class="panel cookie-panel">
      ...
      <div v-if="cookieExpanded" id="cookie-health-detail" class="cookie-detail">
        <div v-if="cookieAccounts.length" class="table-wrap"><table class="data-table cookie-table"><thead><tr><th>账号</th><th>SteamID</th><th>Cookie 状态</th><th>Steam 币种</th><th>最近验证</th><th>最近刷新结果</th><th>失败次数</th><th>下次动作</th></tr></thead><tbody><tr v-for="account in cookieAccounts" :key="account.accountId || account.steamId"><td>{{ account.accountName || account.name || "未命名" }}</td><td class="mono">{{ account.steamId || "—" }}</td><td><span :class="['pill', account.valid ? 'success' : account.status === 'unknown' ? 'neutral' : 'warning']">{{ account.valid ? "有效" : account.status || "未知" }}</span></td><td><span :class="['pill', account.currencyStatus === 'cny' ? 'success' : account.currencyStatus === 'non_cny' ? 'warning' : 'neutral']">{{ account.currencyStatus === 'cny' ? `${account.currency || 'CNY'} · ID ${account.currencyId ?? 23}` : account.currencyStatus === 'non_cny' ? `${account.currency || '非人民币'} · ID ${account.currencyId ?? '?'}` : '未知' }}</span><small v-if="account.currencyCheckedAt">{{ formatLocal(account.currencyCheckedAt) }}</small></td><td>{{ formatLocal(account.lastCheckedAt) }}</td><td :class="{ 'danger-text': account.error || account.currencyError }">{{ account.error || account.currencyError || account.lastResult || "—" }}</td><td>{{ account.failureCount || 0 }}</td><td>{{ account.nextRetryAt ? formatCountdown(account.nextRetryAt) : "运行中监测" }}</td></tr></tbody></table></div>
    ...
    <section class="panel scan-panel">
      ...
      <article v-if="dashboard?.currentScan" class="current-scan-card">
        <div class="current-scan-title"><span></span><strong>本轮扫描：运行中</strong></div>
        <div><small>开始时间</small><b>{{ formatLocal(dashboard.currentScan.startedAt) }}</b></div>
        <div><small>候选行情</small><b>{{ dashboard.currentScan.evaluatedCount || 0 }} 项已读取</b></div>
        <div><small>符合比例</small><b>{{ dashboard.currentScan.candidateCount || 0 }} 项</b></div>
        <div><small>新上架</small><b>{{ dashboard.currentScan.listedCount || 0 }} 笔</b></div>
        <div class="current-scan-step"><small>当前步骤</small><b>{{ dashboard.currentScan.currentStep || "正在处理" }}</b></div>
      </article>
