...
export type AuditVerdict = "passed" | "failed" | "inconclusive";
export type AuditDataset = "steamSales" | "rebuyChains" | "itemConservation" | "wallet";
export type AuditStageKey = "local" | "steam" | "c5" | "matching" | "summary";
export type AuditStageState = "pending" | "current" | "completed" | "failed" | "cancelled";
export type AuditCellFormat = "text" | "integer" | "money" | "moneyFen" | "percent" | "datetime" | "evidence";
...
export interface AuditFormState {
  ...
  openingRealValue: string;
  accountIds: string[];
}

export interface AuditAccountOption {
  id: string;
  label: string;
  steamId: string | null;
}
...
export interface AuditEvidenceGap {
  source: string;
  code: string;
  message: string;
  state: string;
...
export interface AuditRun {
  ...
  stage: AuditStageKey | null;
  progress: AuditProgress;
  summary: AuditRow;
  coverage: AuditRow;
  evidenceGaps: AuditEvidenceGap[];
...
export interface AuditTableColumn {
  ...
  tone?: "default" | "strong" | "difference";
  digits?: number;
}
...
export const AUDIT_STAGES: ReadonlyArray<{ key: AuditStageKey; index: number; label: string; hint: string }> = [
...
export const AUDIT_TABLES: ReadonlyArray<AuditTableDefinition> = [
  {
    dataset: "steamSales",
...
const STAGE_ALIASES: Record<string, AuditStageKey> = {
  local: "local",
  database: "local",
  local_records: "local",
  localrecords: "local",
...
const DATASET_SECTIONS: Record<AuditDataset, string> = {
...
function recordFrom(value: unknown): AuditRow {
  return isRecord(value) ? value : {};
}

function nestedRecord(root: AuditRow, keys: readonly string[]): AuditRow {
...
export function firstValue(root: AuditRow, aliases: readonly string[]): unknown {
...
function safePercent(value: unknown, done: number, total: number): number {
  const explicit = finiteNumber(value);
  if (explicit !== null) {
    const normalized = explicit <= 1 && explicit >= 0 ? explicit * 100 : explicit;
    return Math.min(100, Math.max(0, normalized));
  }
  if (total > 0) return Math.min(100, Math.max(0, (done / total) * 100));
  return 0;
}

export function toBeijingDateTimeInput(value: Date = new Date()): string {
...
export function toBeijingApiTimestamp(value: string): string {
...
export function normalizeDecimalInput(value: unknown, fallback: string): string {
...
export function defaultAuditForm(now: Date = new Date()): AuditFormState {
  return {
...
export function normalizeAuditPreset(payload: unknown, now: Date = new Date()): AuditPreset {
    ...
    : Array.isArray(source.steamAccounts)
      ...
      : Array.isArray(source.accountIds)
        ...
        : Array.isArray(source.steamAccountIds)
          ...
          : [];
  const accounts = rawAccounts.flatMap((entry): AuditAccountOption[] => {
  ...
  return {
    ...
    openingWallet: normalizeDecimalInput(
      firstValue(source, ["openingWallet", "openingWalletCny", "openingSteamBalance", "initialBalance"]),
      defaults.openingWallet,
    ),
    openingRealValue: normalizeDecimalInput(
      firstValue(source, ["openingRealValue", "openingRealValueCny", "openingCashValue", "initialRealValue"]),
      defaults.openingRealValue,
    ),
    accounts,
  };
}

export function validateAuditForm(form: AuditFormState): string[] {
  const errors: string[] = [];
  const start = Date.parse(toBeijingApiTimestamp(form.dateFrom));
  const end = Date.parse(toBeijingApiTimestamp(form.dateTo));
  if (!form.dateFrom || Number.isNaN(start)) errors.push("开始时间无效");
  if (!form.dateTo || Number.isNaN(end)) errors.push("结束时间无效");
  if (!Number.isNaN(start) && !Number.isNaN(end) && end <= start) errors.push("结束时间必须晚于开始时间");
  const wallet = finiteNumber(form.openingWallet);
  const realValue = finiteNumber(form.openingRealValue);
  if (wallet === null || wallet < 0) errors.push("期初 Steam 账面余额无效");
  if (realValue === null || realValue < 0) errors.push("期初余额真实价值无效");
  return errors;
}

export function buildAuditRunRequest(form: AuditFormState): AuditRow {
  const dateFrom = toBeijingApiTimestamp(form.dateFrom);
  const dateTo = toBeijingApiTimestamp(form.dateTo);
  const accountIds: string[] | "all" = form.accountIds.length ? [...form.accountIds] : "all";
  return {
    dateFrom,
    dateTo,
    startAt: dateFrom,
...
export function normalizeAuditStatus(value: unknown): AuditStatus {
  ...
  if (["running", "processing", "retrying", "collecting"].includes(status)) return "running";
  if (["completed", "complete", "done", "success", "passed", "inconclusive", "partial", "completed_with_gaps", "completedwithgaps"].includes(status)) return "completed";
  if (["failed", "error"].includes(status)) return "failed";
  if (["cancelled", "canceled"].includes(status)) return "cancelled";
  return "queued";
}

export function normalizeAuditVerdict(value: unknown): AuditVerdict | null {
  const verdict = stringValue(value).toLowerCase().replace(/[\s-]+/g, "_");
  if (["passed", "pass", "success", "verified"].includes(verdict)) return "passed";
  if (["failed", "fail", "mismatch"].includes(verdict)) return "failed";
  if (["inconclusive", "unknown", "partial", "completed_with_gaps", "unverified"].includes(verdict)) return "inconclusive";
  return null;
}

export function normalizeAuditStage(value: unknown): AuditStageKey | null {
  const key = stringValue(value).toLowerCase().replace(/[\s-]+/g, "_");
  return STAGE_ALIASES[key] || null;
}

export function isActiveAuditStatus(status: AuditStatus | null): boolean {
...
function normalizeEvidenceGap(value: unknown): AuditEvidenceGap | null {
...
function normalizeEvidenceGaps(value: unknown): AuditEvidenceGap[] {
  if (!Array.isArray(value)) return [];
  return value.map(normalizeEvidenceGap).filter((gap): gap is AuditEvidenceGap => gap !== null);
}

export function extractEvidenceGaps(row: AuditRow): AuditEvidenceGap[] {
...
export function normalizeAuditRun(payload: unknown, fallbackRequestId = ""): AuditRun {
  const outer = recordFrom(payload);
  ...
  const percent = status === "completed" ? 100 : Math.max(stagePercent, safePercent(
    firstValue(progressRecord, ["percent", "percentage", "ratio"]),
    done,
    total,
  ));
  return {
...
export function summaryNumber(summary: AuditRow, aliases: readonly string[]): number | null {
  return finiteNumber(firstValue(summary, aliases));
}

export function stageState(stage: AuditStageKey, run: AuditRun | null): AuditStageState {
  ...
  if (run.status === "failed") return targetIndex === Math.max(0, currentIndex) ? "failed" : targetIndex < currentIndex ? "completed" : "pending";
  if (run.status === "cancelled") return targetIndex === Math.max(0, currentIndex) ? "cancelled" : targetIndex < currentIndex ? "completed" : "pending";
  if (currentIndex < 0) return targetIndex === 0 && run.status === "running" ? "current" : "pending";
...
export function formatMoneyFen(value: unknown): string {
  const parsed = finiteNumber(value);
...
export function evidenceLabel(value: unknown): string {
  ...
  if (["verified", "matched", "passed", "success", "official"].includes(state)) return "已验证";
  if (["mismatch", "failed", "conflict", "duplicate"].includes(state)) return "存在差异";
  if (["manual", "manual_external"].includes(state)) return "人工证据";
  if (["unknown", "unverified", "inconclusive", "partial", "missing"].includes(state)) return "证据不完整";
  return state ? stringValue(value) : "待核验";
}

export function evidenceTone(value: unknown): "success" | "danger" | "warning" | "neutral" {
  const label = evidenceLabel(value);
  if (label === "已验证") return "success";
  if (label === "存在差异") return "danger";
  if (label === "证据不完整" || label === "人工证据") return "warning";
  return "neutral";
}

export function auditCellValue(row: AuditRow, column: AuditTableColumn): unknown {
  return firstValue(row, column.aliases);
}

export function formatAuditCell(row: AuditRow, column: AuditTableColumn): string {
  ...
  if (column.format === "moneyFen") return formatMoneyFen(value);
  if (column.format === "money") {
    if (matchedAlias && /(Fen|Cents|Minor)$/i.test(matchedAlias)) return formatMoneyFen(value);
  ...
  if (column.format === "evidence") return evidenceLabel(value);
  if (Array.isArray(value)) return value.length ? `${value.length} 项` : "—";
  if (typeof value === "boolean") return value ? "是" : "否";
  return stringValue(value) || "—";
}

export function auditRowKey(row: AuditRow, dataset: AuditDataset, index: number): string {
...
export function buildAuditRowsUrl(requestId: string, dataset: AuditDataset, page: number, pageSize: number): string {
...
export function buildAuditExportUrl(requestId: string, format: "json" | "csv" | "markdown"): string {
  const query = new URLSearchParams({ format });
  return `/api/guadao-audit/runs/${encodeURIComponent(requestId)}/export?${query.toString()}`;
}

export function verdictCopy(verdict: AuditVerdict | null): { label: string; title: string; description: string } {
  if (verdict === "passed") {
